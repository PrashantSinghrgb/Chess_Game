...
namespace Game
{
    public class Player : MonoBehaviour
    {
		...
		public void AddCapturedPiece(BoardPiece piece)
		{
			...
			//Arrange Pieces Horizontally
			/*//determine (x, y) position and place just off the board
			int x = numberCapturedPieces % GameBoard.COLUMS;
			x = (isWhite ? x : GameBoard.COLUMS - 1 - x);
			float y = (numberCapturedPieces / GameBoard.COLUMS) * (isWhite ? -1 : 1);
			piece.transform.localPosition = new Vector3(x, y, 0f);*/

			//Arraning pieces vertically
			//determine (x, y) position and place just off the board
			int y = numberCapturedPieces % GameBoard.ROWS; // using ROWS instead of COLUMS for vertical arranging
			y = (isWhite ? y : GameBoard.ROWS - 1 - y); // Adjust for player color
			float x = (numberCapturedPieces / GameBoard.ROWS) * (isWhite ? 1 : -1);// Adjust x position for veritcal arrangement
			piece.transform.localPosition = new Vector3(x, y, 0f);
